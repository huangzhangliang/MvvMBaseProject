 ...
 * @date 2017/5/16.
 */
public class ListFragment extends BaseFragment<FragmentHomeBinding>{
        ...
        mTestPresenter = new TestPresenter(new PresenterCallback<HomeModelInfo>() {
            ...
            public void onError(Exception e) {
                ...
                UIUtils.postTaskDelay(new Runnable() {
                    @Override
                    public void run() {
                        // 没有正确的网络请求，只能在错误方法中模拟网络数据回调 如果Demo上拉加载出现延迟3秒刷新问题就是在这里
                        // 真正的网络数据在这里
                        final List<UserModel> data2 = new ArrayList<>();
                        UserModel userModel = new UserModel();
                        userModel.text1 = "1111111";
                        userModel.text2 = "2222222";
                        userModel.text3 = "3333333";
                        userModel.type = 2;
                        data2.add(userModel);
                        data2.add(userModel);
                        data2.add(userModel);
                        ...
                        data2.add(userModel);
                        data2.add(userModel);
                        UserModel userModel2 = (UserModel) mAdapter.getData().get(1);
                        if (userModel2.type == 1){
        ...
        mAdapter = new DataBindRecyclerViewAdapter<UserModel,ViewDataBinding>() {
            ...
            public void onBindHolder(ViewDataBinding binding, UserModel itemData, int position) {
                if (getData().get(position).type == 0){
                    ...
                    LayoutLoadingBinding loadingBinding = (LayoutLoadingBinding) binding;
                    ViewGroup.LayoutParams layoutParams = loadingBinding.superContainer.getLayoutParams();
                    layoutParams.height = viewBinding.trl.getHeight() - viewBinding.headerLayout.superContainer.getHeight();
                    loadingBinding.superContainer.setLayoutParams(layoutParams);
                    loadingBinding.progressWheel.requestLayout();
                    loadingBinding.btnErrorRetry.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            // 点击错误按钮时重新加载数据
                            viewBinding.trl.startRefresh();
                        }
                    });
                }else {
